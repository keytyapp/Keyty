...
Keyty needs macOS `Accessibility` permission to capture and visualize input events in non-App-Store builds, including when running a debug build from Xcode. App Store builds use `Input Monitoring` instead.
...
- No keyboard or mouse events appear:
  Check macOS permissions first. Old app references in **Accessibility** (or **Input Monitoring** for the App Store build) are a common cause.
